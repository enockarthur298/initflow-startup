...
export const Menu = () => {
  ...
  const deleteItem = useCallback(
    (event: React.UIEvent, item: ChatHistoryItem) => {
      ...
      console.log('Attempting to delete Project:', { id: item.id, description: item.description });
  ...
  const deleteSelectedItems = useCallback(
    async (itemsToDeleteIds: string[]) => {
      ...
      if (errors.length === 0) {
        toast.success(`${deletedCount}project${deletedCount === 1 ? '' : 's'} deleted successfully`);
      } else {
  ...
  useEffect(() => {
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      // Only auto-open on mouse hover if user is signed in or sidebar was manually opened before
      if (event.pageX < enterThreshold && (isSignedIn || manuallyOpened)) {
        setOpen(true);
      }
  ...
  }, [isSettingsOpen, isSignedIn, manuallyOpened]);

  // Add a toggle button to manually open the sidebar
  const toggleSidebar = useCallback(() => {
    // Only allow signed-in users to toggle the sidebar
    if (isSignedIn) {
      setOpen(!open);
      setManuallyOpened(true);
    } else {
      // For non-signed-in users, show a toast notification
      toast.info('Please sign in to access your projects', {
        position: 'bottom-right',
        autoClose: 3000,
      });
    }
  ...
  return (
    <>
      {/* Repositioned toggle button that's always visible when sidebar is closed */}
      {!open && (
        <button
          onClick={toggleSidebar}
          className="fixed top-[calc(var(--header-height)+8px)] left-0 z-40 bg-white dark:bg-gray-900 p-2 rounded-r-lg shadow-md border border-l-0 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          aria-label="Open sidebar"
        >
          <div className="i-ph:sidebar text-gray-700 dark:text-gray-300 w-5 h-5" />
        </button>
      )}
      ...
      >
        {/* Close button inside the sidebar has been removed */}
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="flex items-center justify-between text-sm px-4 py-2">
            <div className="font-medium text-gray-600 dark:text-gray-400">Projects</div>
            {selectionMode && (
            ...
            <DialogRoot open={dialogContent !== null}>
              ...
              <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                ...
                {dialogContent?.type === 'bulkDelete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-gray-950">
                      <DialogTitle className="text-gray-900 dark:text-white">Delete Selected projects?</DialogTitle>
                      <DialogDescription className="mt-2 text-gray-600 dark:text-gray-400">
                        <p>
